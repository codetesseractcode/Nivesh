...
export class HealthController {
   ...
   */
  @Get('live')
  @ApiOperation({ summary: 'Kubernetes liveness probe — fast, no dependency checks' })
  ...
  async checkReadiness(): Promise<HealthCheckResult> {
    ...
    const [postgres, redis, neo4j, mongodb, clickhouse] = await Promise.allSettled([
      ...
      this.timedCheck('clickhouse', () => this.clickhouseService.healthCheck()),
    ]);
    ...
    if (overallStatus === 'unhealthy') {
      this.logger.error('Readiness check FAILED — dependencies unhealthy', checks);
      throw new ServiceUnavailableException(response);
    }

    if (overallStatus === 'degraded') {
      this.logger.warn('Readiness check DEGRADED — some dependencies unhealthy', checks);
    }
  ...
  private async timedCheck(
    ...
    try {
      ...
      return { status: 'down', responseTime, error: `${name} returned unhealthy` };
    } catch (error) {
      const responseTime = Date.now() - start;
      return {
        status: 'down',
