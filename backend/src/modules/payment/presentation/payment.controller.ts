...
import { CreatePaymentDto, VerifyPaymentDto, PaymentResponseDto } from '../application/dto';
import {
...
import { GetPaymentQuery, GetUserPaymentsQuery, GetPaymentsByStatusQuery } from '../application/queries';
...
export class PaymentController {
  ...
  async verifyPayment(
    ...
    const userId = req.user?.sub || req.user?.userId || 'anonymous';
    const command = new VerifyPaymentCommand(paymentId, userId, verifyPaymentDto);
    return this.commandBus.execute(command);
  }

  @Get()
  @ApiOperation({ summary: 'Get all payments for the current user (paginated)' })
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'Filtered payments retrieved' })
  ...
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: 'Payment cannot be cancelled' })
